(() => {
  ...
  function initBlogSearch() {
    ...
    const entries = Array.from(document.querySelectorAll('.blog-index-grid .blog-card')).map((card) => {
      const link = card.querySelector('.read-more') || card.querySelector('.blog-card-media');
      return {
        card,
        href: link ? link.href : '',
        text: normalizeSearchText(card.textContent)
      };
    });
    const total = entries.length;
    let articleIndexPromise = null;

    function updateCount(visible, hasQuery) {
      count.innerHTML = hasQuery
        ? `<span class="lang-pl-content">${visible} z ${total} artykułów</span><span class="lang-en-content">${visible} of ${total} articles</span>`
        : `<span class="lang-pl-content">${total} artykułów</span><span class="lang-en-content">${total} articles</span>`;
    }
    ...
    function applySearch() {
      ...
      entries.forEach((entry) => {
        const matches = tokens.length === 0 || tokens.every((token) => entry.text.includes(token));
        entry.card.hidden = !matches;
        if (matches) visible += 1;
      });

      if (empty) empty.hidden = tokens.length === 0 || visible > 0;
      updateCount(visible, tokens.length > 0);
      if (tokens.length > 0) loadArticleIndex();
    }

    function loadArticleIndex() {
      if (articleIndexPromise || !window.fetch || !window.DOMParser) return articleIndexPromise;
      const parser = new DOMParser();
      articleIndexPromise = Promise.all(entries.map(async (entry) => {
        if (!entry.href) return;
        try {
          const response = await fetch(entry.href, { credentials: 'same-origin' });
          if (!response.ok) return;
          const doc = parser.parseFromString(await response.text(), 'text/html');
          const article = doc.querySelector('.article-body') || doc.querySelector('main') || doc.body;
          entry.text = `${entry.text} ${normalizeSearchText(article ? article.textContent : '')}`;
        } catch (error) {
          // The card title and summary remain searchable if a page cannot be fetched.
        }
      })).then(applySearch);
    ...
    input.addEventListener('focus', loadArticleIndex, { once: true });
    applySearch();
  }
