# Natural Healing - Natalia Corvo

Static bilingual website for Natalia Corvo, a Polish nutrition professional in Dublin.
...
Public URLs are canonicalized without `.html` through Cloudflare static asset HTML handling and `_redirects`. After deployment, submit `https://nataliacorvo.com/sitemap.xml` in Google Search Console and run `npm run indexnow` to notify IndexNow-compatible search engines.
