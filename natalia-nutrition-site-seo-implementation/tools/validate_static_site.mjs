...
const requiredFiles = [
  ...
  "cd0ca4b412175b7c0fe92b10bd43f866.txt",
  "_redirects",
  "robots.txt",
  "sitemap.xml",
  "css/styles.css",
...
function referenceCandidates(rawReference, fromFile) {
  if (
  ...
  if (!path.posix.extname(resolved)) {
    return [resolved, `${resolved}.html`, `${resolved}/index.html`];
  }
  return [resolved];
}
...
function checkHtmlReferences(relativePath) {
  ...
  for (const match of html.matchAll(referencePattern)) {
    const candidates = referenceCandidates(match[1], relativePath);
    if (candidates.length === 0) continue;
    if (!candidates.some(fileExists)) {
      errors.push(`${relativePath} references missing file: ${match[1]} -> ${candidates.join(" | ")}`);
    }
  }
}
...
function checkSitemap() {
  ...
  if (/<loc>https:\/\/nataliacorvo\.com\/[^<]+\.html<\/loc>/.test(sitemap)) {
    errors.push("sitemap.xml contains .html URLs; use canonical extensionless URLs.");
  }
}
...
if (fileExists("sitemap.xml")) checkSitemap();
