...
<section class="page-hero article-hero">
  <div class="container article-hero-grid">
    <div class="stack">
      ...
      <p class="article-meta">${langPair("Opublikowano", "Published")} <time datetime="${article.datePublished}">${formatDatePl(article.datePublished)}</time> · ${langPair("Aktualizacja", "Updated")} <time datetime="${article.dateModified}">${formatDatePl(article.dateModified)}</time></p>
    </div>
...
function renderBlogCard(article, cover) {
  ...
  <div class="card-kicker"><span>${langPair(article.category_pl, article.category_en)}</span><span>${article.reading_time_minutes} min</span></div>
