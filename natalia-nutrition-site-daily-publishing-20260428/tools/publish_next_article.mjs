...
const sourceRootRelative = "data/article-publishing/nataliacorvo_codex_new_articles_sources";
const sourceRoot = path.join(root, sourceRootRelative);
...
async function main() {
  ...
  if (publishMode && !ignoreTime && now.hour !== "08") {
    console.log(`Skipping publish: Europe/Dublin local hour is ${now.hour}, not 08.`);
  ...
  const generated = buildPublication(source, state, sourceBySlug, cover);
...
function assertSourcePackageIntegrity() {
  const docs = [
    "CODEX_PUBLISHING_INSTRUCTIONS.md",
    "ARTICLE_MANIFEST.md",
    "SEO_AI_CRAWLING_RULES.md",
    "ARTICLE_SOURCE_CONTRACT.md",
  ];
  for (const doc of docs) {
    const file = path.join(sourceRoot, doc);
    if (!existsSync(file)) fail(`Missing required source document: ${file}`);
  }

  const sourceHashes = readJson(sourceHashesPath);
  if (sourceHashes.zip_file !== "nataliacorvo_codex_new_articles_sources.zip") {
    fail("source_hashes.json does not describe the expected ZIP source package.");
  }
  if (!sourceHashes.files || typeof sourceHashes.files !== "object" || Array.isArray(sourceHashes.files)) {
    fail("source_hashes.json must contain a files object.");
  }

  const expectedFiles = new Set(Object.keys(sourceHashes.files));
  for (const relativeFile of expectedFiles) {
    const filePath = path.join(sourceRoot, relativeFile);
    if (!existsSync(filePath)) fail(`Source package file is missing: ${relativeFile}`);
    const actualHash = sha256File(filePath);
    if (actualHash !== sourceHashes.files[relativeFile]) {
      fail(`Source package file hash mismatch: ${relativeFile}`);
    }
  }

  for (const relativeFile of listFiles(sourceRoot)) {
    if (!expectedFiles.has(relativeFile)) {
      fail(`Unexpected file in committed source package: ${relativeFile}`);
    }
  }
}

function parseManifest() {
  const manifestPath = path.join(sourceRoot, "ARTICLE_MANIFEST.md");
  const text = readText(manifestPath);
  const entries = [];

  for (const line of text.split(/\r?\n/)) {
    const match = line.match(/^\|\s*(\d+)\s*\|\s*`([^`]+)`\s*\|.*?\|\s*(READY_FOR_CODEX)\s*\|\s*`([^`]+)`\s*\|$/);
    if (!match) continue;
    entries.push({
  ...
  if (entries.length !== 24) fail(`Expected 24 manifest entries; found ${entries.length}.`);
  const seen = new Set();
...
function loadSource(entry) {
  const filePath = path.join(sourceRoot, entry.sourceFile);
  if (!existsSync(filePath)) fail(`Missing source article: ${entry.sourceFile}`);
  ...
  const plFaq = extractFaq(parts.plMarkdown);
  const enFaq = extractFaq(parts.enMarkdown);
...
function buildPublication(source, state, sourceBySlug, cover) {
  ...
  const cardHtml = renderBlogCard(source, cover);
  const currentBlogHtml = readText(blogPath);
  const blogHtml = insertBlogCard(currentBlogHtml, cardHtml, source.meta.slug);
...
function renderArticlePage({ source, cover, articleLinks, plQuick, enQuick }) {
  const { meta } = source;
  ...
  const datePl = formatDatePl(meta.datePublished);
  const modifiedPl = formatDatePl(meta.dateModified);
...
<section class="page-hero article-hero">
  <div class="container article-hero-grid">
    <div class="stack">
      ...
      <p class="article-meta"><span class="lang-pl-content">Opublikowano</span><span class="lang-en-content">Published</span> <time datetime="${meta.datePublished}">${datePl}</time> · <span class="lang-pl-content">Aktualizacja</span><span class="lang-en-content">Updated</span> <time datetime="${meta.dateModified}">${modifiedPl}</time></p>
    </div>
...
function renderFooter(prefix) {
  ...
  <div class="container footer-grid">
    <div>
      ...
      <p><span class="lang-pl-content">Wsparcie żywieniowe Natalii Wcisło w Dublinie i online. Treści na stronie mają charakter edukacyjny i nie zastępują indywidualnej diagnozy medycznej.</span><span class="lang-en-content">Nutrition support by Natalia Wcisło in Dublin and online. Website content is educational and does not replace individual medical diagnosis.</span></p>
    </div>
    <div>
...
function renderInlineMarkdown(value) {
  return escapeHtml(value)
    ...
    .replace(/&lt;br&gt;/g, "<br>")
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_match, text, href) => `<a href="${escapeAttribute(href)}">${escapeHtml(text)}</a>`);
}
...
function renderInlineLinks(links) {
  const plLinks = links.map((link) => `<a href="${escapeAttribute(link.slug)}">${escapeHtml(link.title_pl)}</a>`).join(", ");
  const enLinks = links.map((link) => `<a href="${escapeAttribute(link.slug)}">${escapeHtml(link.title_en)}</a>`).join(", ");
...
function renderReadAlso(links) {
  return `<ul class="related-list">${links.map((link) => `<li><a href="${escapeAttribute(link.slug)}"><span class="lang-pl-content">${escapeHtml(link.title_pl)}</span><span class="lang-en-content">${escapeHtml(link.title_en)}</span></a></li>`).join("")}</ul>`;
}

function renderBlogCard(source, cover) {
  const { meta } = source;
  return `<article class="card blog-card" data-article-slug="${escapeAttribute(meta.slug)}">
  <a class="blog-card-media" href="articles/${escapeAttribute(meta.slug)}" aria-label="${escapeAttribute(meta.title_pl)}">
  ...
  <div class="card-kicker"><span><span class="lang-pl-content">${escapeHtml(meta.category_pl)}</span><span class="lang-en-content">${escapeHtml(meta.category_en)}</span></span><span>10 min</span></div>
  ...
  <a class="read-more" href="articles/${escapeAttribute(meta.slug)}"><span class="lang-pl-content">Czytaj artykuł</span><span class="lang-en-content">Read article</span></a>
</article>`;
}
...
function validateGeneratedPublication(generated, source) {
  const { articleHtml, blogHtml } = generated;
  const { meta } = source;
...
function validateState(state, manifest) {
  if (state.version !== 1) fail("state.json has an unsupported version.");
  if (state.source_root !== sourceRootRelative) fail("state.json source_root does not match the configured source path.");
...
function extractFaq(markdown) {
  const faqIndex = headingIndex(markdown, "FAQ");
  if (faqIndex === -1) return [];
  const faqText = markdown.slice(faqIndex).replace(/^## FAQ\s*/u, "").trim();
  const questions = [];
